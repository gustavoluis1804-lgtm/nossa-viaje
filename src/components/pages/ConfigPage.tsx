...
import { hashPassword } from "@/data/trip";
  ...
  return (
    <div className="pb-32">
      ...
      <section className="animate-fade-up mt-7 px-5" style={stagger(2)}>
        ...
        <div className="card divide-y divide-white/[0.05] !shadow-none">
          ...
          <Row
            ...
            subtitle="alterar a senha da surpresa"
            onClick={() => setEditing({ kind: "password" })}
          />
        </div>
      </section>
        ...
        <div className="card divide-y divide-white/[0.05] !shadow-none">
          {sched.items.map((item) => (
            <button
              ...
              onClick={() => setEditing({ kind: "item", id: item.id })}
            >
      ...
      <PasswordEditor
        ...
        onClose={() => setEditing(null)}
        onSave={(v) => {
          updateSettings({ passwordHash: hashPassword(v) });
          pushToast("Palavra secreta atualizada");
        }}
      />
      <ItemEditor
